...
import com.example.posdaming.json.SupplierJson;
...
public class StockOutOutsourceActivity extends BaseActivity implements View.OnClickListener, StockInOtherAdapter.OnEditChangeListener, BillTypePopup.OnItemClickListener {

    private LinearLayout ll_scan;
    private TextView tv_scan_select;
    private LinearLayout ll_handle;
    private TextView tv_handle;
    private String TAG = "StockOutOutsourceActivity";
    ...
    //是否保存
    private boolean isSaveOutOutsource = false;
    ...
    private EditText edt_conn_no1;
    private EditText edt_conn_no2;
    ...
    protected void initView(Bundle savedInstanceState) {
        setContentView(R.layout.activity_stock_out_outsource);
        ...
        rv_content = (RecyclerView) findViewById(R.id.rv_content);
        edt_conn_no1 = (EditText) findViewById(R.id.edt_conn_no1);
        edt_conn_no2 = (EditText) findViewById(R.id.edt_conn_no2);
    ...
    protected void initData() {
        ...
        isSaveOutOutsource = SharePreUtils.getBoolean("isSaveOutOutsource", false, this);


        if (isSaveOutOutsource) {
            //mOrderSave = SharePreUtils.getString("orderOutOutsource", "", this);
            mSaveLists = DataSupport.where("type=?", "orderOutOutsource").find(Material.class);
...
//        tv_bill_type.setText("成品入库");
        ...
        x.http().post(params, new Callback.CommonCallback<String>() {
            ...
            public void onSuccess(String result) {
                ...
                if (wareListJson.isIs_success()) {
                    ...
                    ToastUtil.show("获取交货单位失败,请重新获取", StockOutOutsourceActivity.this);
                }
            }
    ...
    private void getSupplier() {
        RequestParams params = new RequestParams(URL.HOST);
        params.addBodyParameter(URL.ACCESS_KEY, URL.ACCESS_VALUE);
        params.addBodyParameter("method", "supplier.list.get");
        x.http().post(params, new Callback.CommonCallback<String>() {
            @Override
            public void onSuccess(String result) {
                Log.e(TAG, result);
                SupplierJson supplierJson = new Gson().fromJson(result, SupplierJson.class);
                if (supplierJson.isIs_success()) {
                    List<SupplierJson.DataBean> supplierJsons = supplierJson.getData();
                    for (int i = 0; i < supplierJsons.size(); i++) {
                        mCompanies.add(new BillType(supplierJsons.get(i).getSupplier_id(), supplierJsons.get(i).getSupplier_name()));
                        if (i == 0) {
                            tv_company.setText(supplierJsons.get(i).getSupplier_name());
                        }
                    }
                } else {
                    ToastUtil.show("获取交货单位失败,请重新获取", StockOutOutsourceActivity.this);
...
//                    ToastUtil.show("扫描失败,请重新扫描", StockOutOutsourceActivity.this);
//                }
//
//            }
//
...
//    }
    ...
    /**
     * user_id创建人id，类型：int
     * supplier_id加工单位id，类型：int
     * ware_id发料仓库id，类型：int
     * conn_no1入库单号，类型：string
     * conn_no2加工源单号，类型：string
    ...
    private void submit() {
        ...
        params.addParameter("method", "placing.buy.add");
        params.addParameter("user_id", mUserId);
        params.addParameter("supplier_id", mCompanies.get(mCompanyPosition).getId());
        params.addParameter("ware_id", mBillTypes.get(mBillTypePosition).getId());
        params.addParameter("conn_no1", "无");
        params.addParameter("conn_no2", edt_conn_no2.getText().toString());
        ...
        Log.e(TAG,"picking.buy.add   user_id:"+mUserId+",supplier_id:"+mCompanies.get(mCompanyPosition).getId()+",ware_id:"+mBillTypes.get(mBillTypePosition).getId()+",conn_no1:无,conn_no2:"+edt_conn_no2.getText().toString()+",wareitemnums:"+wareitemnums);
        ...
        x.http().post(params, new Callback.CommonCallback<String>() {
            ...
            public void onSuccess(String result) {
                ...
                if (dataJson.isIs_success()) {
                    SharePreUtils.putBoolean("isSaveOutOutsource", false, StockOutOutsourceActivity.this);
                    ToastUtil.show("出库成功", StockOutOutsourceActivity.this);
                    finish();
                } else {
                    if (!TextUtils.isEmpty(dataJson.getMsg())) {
                        ToastUtil.show(dataJson.getMsg(), StockOutOutsourceActivity.this);
                    } else {
                        ToastUtil.show("出库失败", StockOutOutsourceActivity.this);
                    }
                }
            }
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.ll_scan:
                ...
                if (ContextCompat.checkSelfPermission(StockOutOutsourceActivity.this, Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED) {
                    // Do not have the permission of camera, request it.
                    ActivityCompat.requestPermissions(StockOutOutsourceActivity.this, new String[]{Manifest.permission.CAMERA}, REQ_CODE_PERMISSION);
                } else {
            ...
            //保存
            case R.id.btn_save:
                SharePreUtils.putBoolean("isSaveOutOutsource", true, this);
                //SharePreUtils.putString("orderOutOutsource", edt_no.getText().toString(), this);
                DataSupport.deleteAll(Material.class, "type = ?", "orderOutOutsource");
                if (orderitemsBeans.size() > 0) {
                    for (int i = 0; i < orderitemsBeans.size(); i++) {
                        Material material = new Material();
                        material.setType("orderOutOutsource");
            ...
            case R.id.btn_confirm:
                if (orderitemsBeans != null && orderitemsBeans.size() > 0) {
                    //String connNo1 = edt_conn_no1.getText().toString().trim();
                    String connNo2 = edt_conn_no2.getText().toString().trim();
                   if ("".equals(connNo2)) {
                        ToastUtil.show("请输入加工源单号", this);
                    } else {
                        submit();
                    }
                } else {
    ...
    private void startCaptureActivityForResult() {
        Intent intent = new Intent(StockOutOutsourceActivity.this, CaptureActivity.class);
        ...
        switch (requestCode) {
            case REQ_CODE_PERMISSION: {
                if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    ToastUtil.show("请在app设置界面开启照相权限", StockOutOutsourceActivity.this);
                }
            }
            break;
        }
            ...
            case CaptureActivity.REQ_CODE:
                switch (resultCode) {
                    ...
                    case RESULT_CANCELED:
                        ...
                        ToastUtil.show("取消扫描", StockOutOutsourceActivity.this);
                        break;
                }
                break;
        }
        ...
        x.http().post(params, new Callback.CommonCallback<String>() {
            ...
            public void onSuccess(String result) {
                ...
                if (materialItemJson.isIs_success()) {
                    ...
                    ToastUtil.show("没有该物料。", StockOutOutsourceActivity.this);
                }
