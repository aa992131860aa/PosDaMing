...
public class StockInOtherActivity extends BaseActivity implements View.OnClickListener, StockInOtherAdapter.OnEditChangeListener, BillTypePopup.OnItemClickListener {
    ...
    //是否保存
    private boolean isSaveStockOther = false;
    ...
    protected void initView(Bundle savedInstanceState) {
        setContentView(R.layout.activity_stock_in_other);
    ...
    protected void initData() {
        ...
        isSaveStockOther = SharePreUtils.getBoolean("isSaveStockOther", false, this);


        if (isSaveStockOther) {
            //mOrderSave = SharePreUtils.getString("orderSaveOther", "", this);
            mSaveLists = DataSupport.where("type=?", "orderSaveOther").find(Material.class);
        ...
        mBillTypes.add(new BillType(13, "其他入库单"));
        mBillTypes.add(new BillType(15, "退料入库单"));

        tv_bill_type.setText("其他入库单");
        mBillTypePosition = 0;
        mCompanyPosition = 0;
        ...
        x.http().post(params, new Callback.CommonCallback<String>() {
            ...
            public void onSuccess(String result) {
                ...
                if (wareListJson.isIs_success()) {
                    ...
                    ToastUtil.show("获取入库仓库失败,请重新获取", StockInOtherActivity.this);
                }
            }
        ...
        x.http().post(params, new Callback.CommonCallback<String>() {
            ...
            public void onSuccess(String result) {
                ...
                if (companyJson.isIs_success()) {
                    ...
                    ToastUtil.show("获取交货单位失败,请重新获取", StockInOtherActivity.this);
                }
            }
...
//     * 采购单号
//     */
//    private void getOrder1(final String itemNo) {
//        RequestParams params = new RequestParams(URL.HOST);
//        params.addBodyParameter(URL.ACCESS_KEY, URL.ACCESS_VALUE);
...
//                    ToastUtil.show("扫描失败,请重新扫描", StockInOtherActivity.this);
//                }
//
//            }
//
...
//    }
    ...
    private void submit() {
        ...
        params.addParameter("method", "receiving.other.add");
        params.addParameter("user_id", mUserId);
        params.addParameter("ware_id", mBillTypes3.get(mBillTypePosition3).getId());
        params.addParameter("flow_type", mBillTypes.get(mBillTypePosition).getId());
        ...
        x.http().post(params, new Callback.CommonCallback<String>() {
            ...
            public void onSuccess(String result) {
                ...
                if (dataJson.isIs_success()) {
                    SharePreUtils.putBoolean("isSaveStockOther", false, StockInOtherActivity.this);
                    ToastUtil.show("入库成功", StockInOtherActivity.this);
                    finish();
                } else {
                    if (!TextUtils.isEmpty(dataJson.getMsg())) {
                        ToastUtil.show(dataJson.getMsg(), StockInOtherActivity.this);
                    } else {
                        ToastUtil.show("入库失败", StockInOtherActivity.this);
                    }
                }
            }
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.ll_scan:
                ...
                if (ContextCompat.checkSelfPermission(StockInOtherActivity.this, Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED) {
                    // Do not have the permission of camera, request it.
                    ActivityCompat.requestPermissions(StockInOtherActivity.this, new String[]{Manifest.permission.CAMERA}, REQ_CODE_PERMISSION);
                } else {
            ...
            //保存
            case R.id.btn_save:
                SharePreUtils.putBoolean("isSaveStockOther", true, this);
                //SharePreUtils.putString("orderSaveOther", edt_no.getText().toString(), this);
                DataSupport.deleteAll(Material.class, "type = ?", "orderSaveOther");
                if (orderitemsBeans.size() > 0) {
                    for (int i = 0; i < orderitemsBeans.size(); i++) {
                        Material material = new Material();
                        material.setType("orderSaveOther");
    ...
    private void startCaptureActivityForResult() {
        Intent intent = new Intent(StockInOtherActivity.this, CaptureActivity.class);
        ...
        switch (requestCode) {
            case REQ_CODE_PERMISSION: {
                if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    ToastUtil.show("请在app设置界面开启照相权限", StockInOtherActivity.this);
                }
            }
            break;
        }
            ...
            case CaptureActivity.REQ_CODE:
                switch (resultCode) {
                    case RESULT_OK:
                        ...
                        //ToastUtil.show(result, StockInOtherActivity.this);
                        //getOrder(result);
                        getItem(result, -1);
                        break;
                    case RESULT_CANCELED:
                        ...
                        ToastUtil.show("取消扫描", StockInOtherActivity.this);
                        break;
                }
                break;
        }
            ...
            public void onSuccess(String result) {
                ...
                if (materialItemJson.isIs_success()) {
                   ...
                   edt_no.setText(itemNo);
                    ...
                    ToastUtil.show("没有该物料。", StockInOtherActivity.this);
                }
