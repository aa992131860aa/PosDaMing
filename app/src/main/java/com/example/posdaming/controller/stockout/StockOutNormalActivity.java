...
public class StockOutNormalActivity extends BaseActivity implements View.OnClickListener, StockInOtherAdapter.OnEditChangeListener, BillTypePopup.OnItemClickListener {

    private LinearLayout ll_scan;
    private TextView tv_scan_select;
    private LinearLayout ll_handle;
    private TextView tv_handle;
    private String TAG = "StockOutNormalActivity";
    ...
    //是否保存
    private boolean isSaveOutNormal = false;
    ...
    protected void initView(Bundle savedInstanceState) {
        setContentView(R.layout.activity_stock_out_normal);
    ...
    protected void initData() {
        ...
        isSaveOutNormal = SharePreUtils.getBoolean("isSaveOutNormal", false, this);


        if (isSaveOutNormal) {
           // mOrderSave = SharePreUtils.getString("orderOutNormal", "", this);
            mSaveLists = DataSupport.where("type=?", "orderOutNormal").find(Material.class);
        ...
        x.http().post(params, new Callback.CommonCallback<String>() {
            ...
            public void onSuccess(String result) {
                ...
                if (wareListJson.isIs_success()) {
                    ...
                    ToastUtil.show("获取交货单位失败,请重新获取", StockOutNormalActivity.this);
                }
            }
            ...
            public void onSuccess(String result) {
                ...
                if (companyJson.isIs_success()) {
                    ...
                    for (int i = 0; i < compnayJsons.size(); i++) {
                        mCompanies.add(new BillType(compnayJsons.get(i).getOrg_id(), compnayJsons.get(i).getOrg_name(), compnayJsons.get(i).getPid()));
                        if (i == 0) {
...
//                    ToastUtil.show("扫描失败,请重新扫描", StockOutNormalActivity.this);
//                }
//
//            }
//
...
//    }
     ...
     * user_id创建人id，类型：int
     * org_id领料部门id，类型：int
     * picker_id领料人id，类型：int
     * ware_id发料仓库id，类型：int
     * conn_no源单单号，类型：string
    ...
    private void submit() {
        ...
        params.addParameter("method", "picking.add");
        params.addParameter("user_id", mUserId);
        params.addParameter("org_id", mCompanies.get(mCompanyPosition).getId());
        params.addParameter("picker_id", mUserId);
        params.addParameter("ware_id", mBillTypes.get(mBillTypePosition).getId());
        params.addParameter("conn_no", edt_conn_no.getText().toString());
        ...
        x.http().post(params, new Callback.CommonCallback<String>() {
            ...
            public void onSuccess(String result) {
                ...
                if (dataJson.isIs_success()) {
                    SharePreUtils.putBoolean("isSaveOutNormal", false, StockOutNormalActivity.this);
                    ToastUtil.show("出库成功", StockOutNormalActivity.this);
                    finish();
                } else {
                    if(!TextUtils.isEmpty(dataJson.getMsg())) {
                        ToastUtil.show(dataJson.getMsg(), StockOutNormalActivity.this);
                    }else{
                        ToastUtil.show("出库失败", StockOutNormalActivity.this);
                    }
                }
            }
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.ll_scan:
                ...
                if (ContextCompat.checkSelfPermission(StockOutNormalActivity.this, Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED) {
                    // Do not have the permission of camera, request it.
                    ActivityCompat.requestPermissions(StockOutNormalActivity.this, new String[]{Manifest.permission.CAMERA}, REQ_CODE_PERMISSION);
                } else {
            ...
            //保存
            case R.id.btn_save:
                SharePreUtils.putBoolean("isSaveOutNormal", true, this);
                //SharePreUtils.putString("orderOutNormal", edt_no.getText().toString(), this);
                DataSupport.deleteAll(Material.class, "type = ?", "orderOutNormal");
                if (orderitemsBeans.size() > 0) {
                    for (int i = 0; i < orderitemsBeans.size(); i++) {
                        Material material = new Material();
                        material.setType("orderOutNormal");
    ...
    private void startCaptureActivityForResult() {
        Intent intent = new Intent(StockOutNormalActivity.this, CaptureActivity.class);
        ...
        switch (requestCode) {
            case REQ_CODE_PERMISSION: {
                if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    ToastUtil.show("请在app设置界面开启照相权限", StockOutNormalActivity.this);
                }
            }
            break;
        }
            ...
            case CaptureActivity.REQ_CODE:
                switch (resultCode) {
                    case RESULT_OK:
                        ...
                        //ToastUtil.show(result, StockOutNormalActivity.this);
                        getItem(result, -1);
                    ...
                    case RESULT_CANCELED:
                        ...
                        ToastUtil.show("取消扫描", StockOutNormalActivity.this);
                        break;
                }
                break;
        }
        ...
        x.http().post(params, new Callback.CommonCallback<String>() {
            ...
            public void onSuccess(String result) {
                ...
                if (materialItemJson.isIs_success()) {
                    ...
                    ToastUtil.show("没有该物料。", StockOutNormalActivity.this);
                }
